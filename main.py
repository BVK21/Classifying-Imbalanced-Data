...
def main():
    ...
    reuters_df = pd.read_pickle("reuters_df.pkl")
    newsgroup_data = pd.read_pickle("newsgroup_data.pkl")
    bbc_data = pd.read_pickle("bbc_data.pkl")
    bbc_imb = pd.read_pickle("bbc_imb.pkl")

    raw = {'BBC':{},'Ruters':{},'20_Newsgroup':{}, 'BBC_org':{}}
    print("Training Original Dataset")
    ...
    with open('oversample.json') as f:
        data = json.load(f)
    plot(data,"Oversampling")

    with open('undersample.json') as f:
        data = json.load(f)
    plot(data,"Undersampling")
